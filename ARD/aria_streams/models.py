...
class VRSStream(models.Model):
    ...
    # 실제 이미지 데이터 필드들 추가
    image_data = models.TextField(null=True, blank=True)  # Base64 인코딩된 이미지
...
class EyeGazeData(models.Model):
    ...
    # 기본 필드 (기존 호환성 유지)
    yaw = models.FloatField()  # Yaw angle in degrees
...
class IMUData(models.Model):
    session = models.ForeignKey(AriaSession, on_delete=models.CASCADE, related_name='imu_data')
    timestamp = models.DateTimeField(default=timezone.now)
    device_timestamp_ns = models.BigIntegerField()
    imu_stream_id = models.CharField(max_length=20)  # e.g., "1202-1" for IMU-left, "1202-2" for IMU-right
    imu_type = models.CharField(max_length=20, choices=[
        ('left', 'IMU Left'),
        ('right', 'IMU Right')
    ])
    ...
    class Meta:
        db_table = 'imu_data'
        indexes = [
            models.Index(fields=['session', 'imu_type', 'device_timestamp_ns']),
            models.Index(fields=['timestamp']),
            models.Index(fields=['imu_stream_id'])
        ]
    
    def __str__(self):
        return f"IMU {self.imu_type} - {self.session.session_id}"
